...
def compute_alpha_precision(
    ...
    Radii = np.quantile(
        torch.sqrt(
            torch.sum((torch.tensor(real_data).float() - emb_center) ** 2, dim=1)
        ),
        alphas,
    )
    synth_to_center = torch.sqrt(
        torch.sum((torch.tensor(synthetic_data).float() - emb_center) ** 2, dim=1)
    )
    ...
    if not alternative_coverage:
        nbrs_synth = NearestNeighbors(n_neighbors=k_coverage, n_jobs=-1, p=2).fit(
            synthetic_data
        )
        ...
        real_synth_closest_d = torch.sqrt(
            torch.sum(
                (torch.tensor(real_synth_closest).float() - synth_center) ** 2, dim=1
            )
        )
        closest_synth_Radii = np.quantile(real_synth_closest_d, alphas)
    else:
        synth_Radii = np.quantile(
            torch.sqrt(
                torch.sum(
                    (torch.tensor(synthetic_data).float() - synth_center) ** 2, dim=1
                )
            ),
            alphas,
        )
        real_to_center = torch.sqrt(
            torch.sum((torch.tensor(real_data).float() - synth_center) ** 2, dim=1)
        )
    ...
    for k in range(len(Radii)):
        ...
        if alternative_coverage:
            beta_coverage = np.mean(
                (real_to_center <= synth_Radii[k]).detach().float().numpy()
            )
        else:
            beta_coverage = np.mean(
                (
    ...
    Delta_precision_alpha = 1 - 2 * np.sum(
        np.abs(np.array(alpha_precision_curve) - alphas)
    ) * (alphas[1] - alphas[0])
    Delta_coverage_beta = 1 - 2 * np.sum(
        np.abs(np.array(beta_coverage_curve) - alphas)
    ) * (alphas[1] - alphas[0])
