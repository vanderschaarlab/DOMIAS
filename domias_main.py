        ...
        for TRAINING_EPOCH in args.training_epoch_list:
            ...
            if args.shifted_column is not None:
                ...
                if reference_kept_p > 0:
                    addition_set = np.concatenate(
                        (addition_set_A1, addition_set_A0_kept), 0
                    )
                    addition_set2 = np.concatenate(
                        (addition_set2_A1, addition_set2_A0_kept), 0
                    )
            ...
            for N_DATA_GEN in args.gen_size_list:
                if args.gan_method == "KDE":
                    samples = pd.DataFrame(
                        kde_model.resample(N_DATA_GEN).transpose(1, 0)
                    )
